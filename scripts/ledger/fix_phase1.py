...
fix_decisions.py; this file only applies them and derives the rest:
 ...
 15. the same rules for every remaining row: 中1〜中3, Pre-Algebra, Algebra 1 / 2,
     Integrated Math and 数B (PHASE2G_*)
...
# ------------------------------------ 10. Phase 2 数列・級数・多変数の単元
def apply_phase2c(rows, log, stats, oos):
    """The same rules for the sequence, series and multivariable units, and the
    three ③ you moved out of scope (fix_decisions PHASE2C_*)."""
    for frm, into, main in D.PHASE2C_SAME:
        merge(rows, frm, into, main, log, action="merged-into")
    for frm, into in D.PHASE2C_SECTION:
        merge(rows, frm, into, "into", log, action="merged-into", names=False)
        note_add(rows[into], f"節の名前「{frm}」を統合（Phase 2 数列・級数の単元）")
    for frm, into in D.PHASE2C_INSTANCE:
        merge(rows, frm, into, "into", log, action="merged-into", names=False)
    for old, new, fields in D.PHASE2C_RENAME:
        rename(rows, old, new, fields, log)
    for frm, into in D.PHASE2C_TO_SYMBOLS.items():
        rows.pop(frm)
        log.append((frm, f"symbols/{into}", "merged-into"))
    phrases = []
    for i in D.PHASE2C_TO_PHRASES:
        r = rows.pop(i)
        phrases.append({"id": i, "ja": r["ja"], "en": r["en"], "pos": r["pos"], "unit": "|".join(r["unit"]),
                        "level_jp": "|".join(r["level_jp"]), "level_us": "|".join(r["level_us"]),
                        "from": "", "note": r["note"]})
        log.append((i, "", "to-phrases"))
    for i, reason in D.PHASE2C_OUT_OF_SCOPE.items():
        r = rows.pop(i)
        oos.append({"id": i, "ja": r["ja"], "en": r["en"], "unit": "|".join(r["unit"]),
                    "level_jp": "|".join(r["level_jp"]), "reason": reason})
        log.append((i, "", "out-of-scope"))
    stats["phase2c"] = {"same": len(D.PHASE2C_SAME), "section": len(D.PHASE2C_SECTION),
                        "instance": len(D.PHASE2C_INSTANCE), "rename": len(D.PHASE2C_RENAME),
                        "to_symbols": len(D.PHASE2C_TO_SYMBOLS), "to_phrases": phrases,
...
# ------------------------------- 12. Phase 2 統計・ベクトル・行列・線形代数の単元
def apply_phase2e(rows, log, stats):
    """The same rules for the statistics, vector, matrix and linear algebra
    units (fix_decisions PHASE2E_*)."""
    for frm, into, main in D.PHASE2E_SAME:
        merge(rows, frm, into, main, log, action="merged-into")
    for frm, into in D.PHASE2E_SECTION:
        merge(rows, frm, into, "into", log, action="merged-into", names=False)
        note_add(rows[into], f"節の名前「{frm}」を統合（Phase 2 統計・ベクトルの単元）")
    for frm, into in D.PHASE2E_INSTANCE:
        merge(rows, frm, into, "into", log, action="merged-into", names=False)
    phrases = []
    for i in D.PHASE2E_TO_PHRASES:
        r = rows.pop(i)
        phrases.append({"id": i, "ja": r["ja"], "en": r["en"], "pos": r["pos"], "unit": "|".join(r["unit"]),
                        "level_jp": "|".join(r["level_jp"]), "level_us": "|".join(r["level_us"]),
                        "from": "", "note": r["note"]})
        log.append((i, "", "to-phrases"))
    stats["phase2e"] = {"same": len(D.PHASE2E_SAME), "section": len(D.PHASE2E_SECTION),
...
# ------------------- 15. Phase 2 中学・Algebra 1／2・Pre-Algebra の単元（残りの行すべて）
def apply_phase2g(rows, log, stats, oos):
    """The same rules for every remaining row (fix_decisions PHASE2G_*). A row
    whose entry is already generated keeps its source and article: the entry
    has its own sources, and a merged row's article is often another concept
    (零点 → Zero (complex analysis) for zeros of a polynomial)."""
    kept_cols = ("source", "wiki_ja", "wiki_en", "wikidata")
    done = {i: {c: r[c] for c in kept_cols} for i, r in rows.items()
            if os.path.exists(os.path.join(ROOT, "data", "terms", f"{i}.json"))}
    for frm, into, main in D.PHASE2G_SAME:
        merge(rows, frm, into, main, log, action="merged-into")
    for frm, into in D.PHASE2G_SECTION:
        merge(rows, frm, into, "into", log, action="merged-into", names=False)
        note_add(rows[into], f"節の名前「{frm}」を統合（Phase 2 中学の単元）")
    for frm, into in D.PHASE2G_INSTANCE:
        merge(rows, frm, into, "into", log, action="merged-into", names=False)
    for old, new, fields in D.PHASE2G_RENAME:
        before = rows[old]["ja"]
        rename(rows, old, new, fields, log)
        if rows[new]["ja"] != before:
            add(rows[new]["ja_alt"], before)
    for i, cols in done.items():
        if i in rows:
            rows[i].update(cols)
    phrases = []
    for i in D.PHASE2G_TO_PHRASES:
        r = rows.pop(i)
        phrases.append({"id": i, "ja": r["ja"], "en": r["en"], "pos": r["pos"], "unit": "|".join(r["unit"]),
                        "level_jp": "|".join(r["level_jp"]), "level_us": "|".join(r["level_us"]),
                        "from": "", "note": r["note"]})
        log.append((i, "", "to-phrases"))
    for i, reason in D.PHASE2G_OUT_OF_SCOPE.items():
        r = rows.pop(i)
        oos.append({"id": i, "ja": r["ja"], "en": r["en"], "unit": "|".join(r["unit"]),
                    "level_jp": "|".join(r["level_jp"]), "reason": reason})
        log.append((i, "", "out-of-scope"))
    stats["phase2g"] = {"same": len(D.PHASE2G_SAME), "section": len(D.PHASE2G_SECTION),
                        "instance": len(D.PHASE2G_INSTANCE), "rename": len(D.PHASE2G_RENAME),
                        "to_phrases": phrases, "out_of_scope": len(D.PHASE2G_OUT_OF_SCOPE)}
...
def transform(en_check=True, langlinks=True, phase2=True):
    ...
    apply_phase2g(rows, log, stats, oos)
    return rows, stats, log, oos
...
def main():
    ...
    big = ("coverage", "check_mismatch", "oos", "dup_en", "en_check", "notation_alias", "notation_headword",
           "langlinks", "phase2", "phase2b", "phase2c", "phase2d", "phase2e", "ap_stats_2026", "phase2f", "phase2g")
    ...
    p2g = stats["phase2g"]
    print(f"phase 2 middle school and the rest (step 15): same concept {p2g['same']}, section names {p2g['section']},"
          f" instances {p2g['instance']}, renamed {p2g['rename']}, to phrases {len(p2g['to_phrases'])},"
          f" out of scope {p2g['out_of_scope']}")
