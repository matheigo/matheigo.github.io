...
- 2026-09-25 | 2 | TERM_FORMS の形に 2 つの記号を足した: 「A | B」（どちらかの形）と「A ! w」（A の直後に w が続かない）。キーにもそのまま残る。decide のレポートの表では | をエスケープする | 1 つの句では別の意味を締め出せない語があった（solve for dx と関連変化率の solve for dx dt、bounded の数列・関数の形が 4 つ）
