...
- 2026-09-24 | 2 | lib.ts の語幹（stem）で、語末の ss を複数形の s として落とさないようにした（compress → compres になり、compressed と合わなかった）。既存のエントリの件数は 1 件も変わらないことを counts.json の前後比較で確かめた。テスト 1 件 | バッチ 2 の compress … horizontally で見つけた。cross ／ express ／ process も同じ形
