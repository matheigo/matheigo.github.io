...
- **NHK 高校講座（数学Ⅰ・数学A・数学Ⅱ・数学B ほか）は使えなかった**（2026-09-26）。https://www.nhk.or.jp/kokokoza/ は https://edu.web.nhk/kokokoza/ に移り、各回の「文字と画像で見る」ページの本文は NHK ONE の「ご利用にあたって」（用途・地域の選択、受信契約の確認）に同意した後にだけ読み込まれる。同意は利用規約への同意に当たるので、エージェントは行わない。人間が同意したうえで使うなら、ページの題名と箇所だけを出典に書く
