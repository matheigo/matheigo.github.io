...
任意項目 `notes`（文字列の配列）: 使うときの注意・米国での扱い（terms の `pitfalls`、symbols の `notes` と同じ役割。2026-09-26 追加）。
...
1. **別セッション**で §8-3 の監査を全コレクションに実施。修正 PR
   - 観点に足す（2026-09-26）: terms の pitfalls・mapping_note にある日本側の主張（「日本の教科書は〜」「日本では〜と書く」）にも、慣習差と同じ基準（日本側を資料で確かめる。学習指導要領・解説、共通テスト・センター試験の問題文、日本語版 Wikipedia）を当てる。確かめられない文は弱めるか消す
...
4. **MICASE**（Michigan Corpus of Academic Spoken English、TalkBank CABank、1997〜2001 年収録。研究・教育目的は無料、商用は許可が必要）: office hours・study group・discussion section など学生の側の発話がある。**phrases の数え方にだけ使い、terms と symbols の判定には入れない**（2026-09-25 追加。docs/SOURCES.md）。
規模の目安: 300〜500 本、300〜500 時間、数百万語。

### コーパス — 書き言葉
OpenStax *Calculus* Vol 1–3、*Precalculus*、*Algebra and Trigonometry*、*Introductory Statistics*、*Prealgebra 2e*、*Elementary Algebra 2e*、*Intermediate Algebra 2e*（CC BY-NC-SA 4.0。後の 3 冊は中学・Algebra の単元のため 2026-09-25 に追加）、MIT OCW の講義ノート。`register: written` の根拠に使う。
